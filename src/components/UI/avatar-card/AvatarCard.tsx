  ...
  return (
    <hgroup className="flex flex-col gap-2 p-2">
      <span className="flex items-center gap-2">
        <img
          className="w-10 h-10 rounded-xl border border-cyan-200"
          src="https://api.dicebear.com/7.x/adventurer/svg?seed=Simba&backgroundColor=b6e3f4"
        />
...
export default function AvatarCard({ imgUrl, name, author, link }: AvatarProps) {
  ...
  return (
    ...
    >
      <span className="hidden lg:flex self-end">
      ...
      <span className="hidden absolute bottom-3 px-4 lg:flex justify-between items-center w-full mt-2">
        <button
          ...
          } lg:flex items-center gap-2 transition-opacity  cursor-pointer`}
        >
      ...
      <hgroup className="w-full flex justify-between lg:hidden">
        <CopyMenu imgUrl={imgUrl} show />
        <DownloadMenu name={name} imgUrl={imgUrl} show />
      </hgroup>
    </div>
  );
}
